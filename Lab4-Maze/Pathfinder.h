...
using namespace std;
	...
	//enum color {BACKGROUND = 1, ABNORMAL = 5, TEMPORARY = 2, PATH = 3, WALL = 0};
...
public:
	Pathfinder() {
		srand(time(0)); //Generates for rest of program
		for(int layer = 0; layer < LAYER_SIZE; layer++){
	        for(int row = 0; row < ROW_SIZE; row++){
	            for(int col = 0; col < COL_SIZE; col++){
	                maze_grid[row][col][layer] = 1;
	            }
	        }
	    }
	}
	...
	*/
	virtual vector<string> solveMaze(/*int grid[ROW_SIZE][COL_SIZE][LAYER_SIZE], int r, int c, int l*/);
	
	bool find_maze_path(int grid[ROW_SIZE][COL_SIZE][LAYER_SIZE], int r, int c, int l);
	void copyMaze(int grid[ROW_SIZE][COL_SIZE][LAYER_SIZE], int grid_copy[ROW_SIZE][COL_SIZE][LAYER_SIZE]);

	//-----------------------------------------------------------------------------------------
	protected:
		int maze_grid[ROW_SIZE][COL_SIZE][LAYER_SIZE]; // To hold values
		int grid_copy[ROW_SIZE][COL_SIZE][LAYER_SIZE];
